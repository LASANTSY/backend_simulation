...
`;
  ...
  public async fetchAndStoreMarkets(bbox: Bbox, providedCity?: string): Promise<Marketplace[]> {
    ...
    // Build node map for centroid calculation and geometry construction
    ...
    for (const el of elements) {
      ...
      if (el.type === 'node') {
        ...
        if (coords.length) {
          ...
          // Build Polygon geometry with real OSM coordinates
          const polygonCoords = coords.map((c: any) => [c.lon, c.lat]);
        ...
        if (coords.length) {
          ...
          // For relations, try to build a MultiPolygon or Polygon
          const polygonCoords = coords.map((c: any) => [c.lon, c.lat]);
          if (polygonCoords.length > 0) {
            const first = polygonCoords[0];
            const last = polygonCoords[polygonCoords.length - 1];
  ...
  existing.city = city || existing.city;
        existing.geometry = geometry || existing.geometry;
